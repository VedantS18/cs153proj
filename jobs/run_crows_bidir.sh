...
#SBATCH --mem=32G
#SBATCH --time=3:00:00
#SBATCH --output=cs153_cbd_%j.out
...
echo "=== Bidirectional sweep with CrowS-derived directions ==="

# Debiasing
echo "--- Debiasing ---"
python scripts/sweep_repe.py \
    --model meta-llama/Llama-3.2-3B \
    --contrast_path results/crows_directions.json \
    --test_dir data/concept_test \
    --out_dir results \
    --out_prefix crows_debias_ \
    --concepts gender_profession gender_emotion age_competence race_crime nationality_stereotype \
    --alphas 0.0 10.0 20.0 40.0 60.0 \
    --layer_frac 0.85 \
    --n_steer_layers 4 \
    --max_new_tokens 50 \
    --max_crows 300 \
